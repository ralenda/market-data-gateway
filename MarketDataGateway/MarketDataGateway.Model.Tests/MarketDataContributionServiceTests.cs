...
public class MarketDataContributionServiceTests
{
    ...
    public async Task Validation_Is_Successful_Stores_Quote()
    {
        ...
        MarketDataContributionRequest request =
            MarketDataContributionRequest.FxContributionRequest(pair!, 0, 0, DateTimeOffset.UtcNow);
        ...
        MarketDataContribution contribution = MarketDataContribution.From(MarketDataContributionId.NewId(),
        ...
        MarketDataContribution result = await service.AddContribution(request);
    ...
    public async Task Validation_Fails_Does_Not_Store_Quote()
    {
        ...
        MarketDataValidation.MarketDataValidation validationResult =
            MarketDataValidation.MarketDataValidation.Failure(MarketDataValidationId.NewId(), "invalid");
        var quoteValidationMock = new Mock<IMarketDataValidationService>();
        quoteValidationMock.Setup(mock => mock.ValidateAsync(request))
            .ReturnsAsync(validationResult);
